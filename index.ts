...
async function getRandomDocumentsV1(collection: Collection<Document>, n: number): Promise<Document[]> {
    const count = await collection.countDocuments();
    const limit = Math.min(n, count);
    return collection.aggregate([{ $sample: { size: limit } }]).toArray();
}

async function getRandomDocumentsV2(collection: Collection<Document>, n: number): Promise<Document[]> {
    const count = await collection.countDocuments();
    const limit = Math.min(n, count);
